...
const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center px-4 py-12 relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(239,68,68,0.15),transparent_30%),radial-gradient(circle_at_bottom_right,rgba(249,115,22,0.12),transparent_25%)]" />
      <div className="w-full max-w-md relative z-10 rounded-[32px] border border-white/10 bg-white/[0.03] p-8 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-white/45 mt-2">Continue learning with your playlist rooms</p>
        </div>
        ...
        <p className="text-white/40 text-sm text-center mt-6">
          Need an account? <Link to={`/register?redirect=${encodeURIComponent(redirectTo)}`} className="text-red-300 hover:text-red-200">Create one</Link>
        </p>
      </div>
    </div>
  );
